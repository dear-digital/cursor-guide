...
import {ShoppingCart} from '@vorwerk/fibre-react';
...
function Badge(props: {cart?: CartApiQueryFragment; count: number}) {
  ...
  return isHydrated ? (
    <Drawer direction={'right'} onOpenChange={setCartOpen} open={cartOpen}>
      <DrawerTrigger className={`${buttonClass} p-0`}>
        {BadgeCounter}
      </DrawerTrigger>
      <DrawerContent
        className="cart flex h-full max-h-screen w-screen flex-col gap-0 bg-background text-foreground lg:left-auto lg:right-0 lg:h-svh max-w-none lg:max-w-3xl"
        onCloseAutoFocus={(e) => e.preventDefault()}
        onOpenAutoFocus={(e) => e.preventDefault()}
      >
        <DrawerHeader className="px-6 py-5 shadow-sm shadow-foreground/10">
          <DrawerTitle className="flex items-center justify-center gap-4 font-body font-bold">
